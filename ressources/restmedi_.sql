...
--
-- Hôte : localhost:3306
-- Généré le : jeu. 14 mars 2024 à 17:18
...
CREATE DEFINER=`gotlub1`@`%` PROCEDURE `majDvd` (IN `idDvd` VARCHAR(10))   BEGIN
    DECLARE nb INTEGER ;
...
CREATE DEFINER=`gotlub1`@`%` PROCEDURE `majLivre` (IN `idLivre` VARCHAR(10))   BEGIN
    DECLARE nb INTEGER ;
    SELECT COUNT(*) INTO nb
        FROM dvd
...
CREATE DEFINER=`gotlub1`@`%` PROCEDURE `majLivresDvd` (IN `idLivreDvd` VARCHAR(10))   BEGIN
    DECLARE nb INTEGER ;
    SELECT COUNT(*) INTO nb
        FROM revue
...
CREATE DEFINER=`gotlub1`@`%` PROCEDURE `majRevue` (IN `idrevue` VARCHAR(10))   BEGIN
    DECLARE nb INTEGER ;
    SELECT COUNT(*) INTO nb
        FROM livres_dvd
        WHERE  id =   idrevue ;
